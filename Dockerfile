...
# Ensure the state directory exists and is writable by the non-root user.
# Cloud platforms (Railway, Render) may not mount a persistent volume at /data,
# so default to a path under the node user's home.
...
# Binds to 0.0.0.0 for cloud platform compatibility (Railway, Render, Fly).
# Auth is required when binding to lan; set OPENCLAW_GATEWAY_TOKEN or
# OPENCLAW_GATEWAY_PASSWORD in your platform's environment variables.
#
...
CMD node openclaw.mjs gateway --allow-unconfigured --bind lan --port ${PORT:-18789}
